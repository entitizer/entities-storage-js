...
var normalizeCreate = exports.normalizeCreate = function(data) {
	...
	var dataToHash = data.data;

	if (data.type === 'create_entity') {
		dataToHash = _.pick(data.data, ['name', 'wikiId', 'englishWikiId']);
	} else if (data.type === 'update_entity') {
		dataToHash = Entity.selectUpdateFields(data.data);
	}

	data.dataHash = createDataHash(dataToHash);
...
var normalizeUpdate = exports.normalizeUpdate = function(data) {
	...
	data = _.pick(data, 'id', 'lang', 'country', 'status', 'entityId', 'resultData', 'oldData');
	...
	// 	data.resultData = JSON.stringify(data.resultData);
	// }

	// if (data.oldData && typeof data.oldData !== 'string') {
	// 	data.oldData = JSON.stringify(data.oldData);
	// }
...
var validateCreate = exports.validateCreate = function(data) {
	...
	if (data.type === 'create_entity') {
		if (data.entityId) {
			throw new Error('Action: `entityId` must be null');
		}
	} else if (!data.entityId) {
		throw new Error('Action: `entityId` is required');
	}
};
