...
Service.prototype.entitiesFromExploreResult = function(result) {
	...
	Object.keys(result).forEach(function(key) {
		...
		if (ids[entity.id]) {
			if (entity.concepts) {
				entities[ids[entity.id] - 1].concepts = entities[ids[entity.id] - 1].concepts.concat(entity.concepts);
			}
		} else {
...
internal.exploreEntities = function(context, concepts, accessService) {
	...
	return accessService.entitiesByNameKeys({
		keys: keys
	}).then(function(entities) {
...
internal.formatConceptsKeys = function(context, concepts) {
	var formatKey = models.EntityName.formatKey;
	var keys = [];
	concepts.forEach(function(concept) {
