...
AccessService.prototype.entitiesByKeys = function(keys, options) {
	...
	return this.entityNamesByKeys(keys, {
		...
		.then(function(entityNames) {
			...
			var entityIds = _.pluck(entityNames, 'entityId');
			entityIds = _.uniq(entityIds);

			return self.entitiesByIds(entityIds, options).then(function(entities) {
				var entity;
				entityNames.forEach(function(item) {
					entity = _.find(entities, 'id', item.entityId);
					if (entity) {
						entity.keys = entity.keys || [];
						entity.keys.push(item.key);
					}
				});
				return entities;
			});
