...
AccessService.prototype.actionById = function(id, options) {
	options = buildOptions(options);
	return vogels.access.getItem('Entitizer_Action', id, options);
};

AccessService.prototype.actionsByIds = function(ids, options) {
	options = buildOptions(options);
	return vogels.access.getItems('Entitizer_Action', ids, options);
};
...
AccessService.prototype.entityNameByKey = function(key, options) {
	options = buildOptions(options);
	return vogels.access.getItem('Entitizer_EntityName', key, options);
};

AccessService.prototype.entityNameByName = function(data, options) {
	var key = EntityName.createKey(data.name, data.lang, data.country);
	return this.entityNameByKey(key, options);
};
...
AccessService.prototype.entityNamesByKeys = function(keys, options) {
	options = buildOptions(options);
	return vogels.access.getItems('Entitizer_EntityName', keys, options);
};
...
AccessService.prototype.entityIdsByEnglishWikiId = function(id, options) {
	...
	return vogels.access.query('Entitizer_Entity', options)
...
AccessService.prototype.entityNameKeysByEntityId = function(entityId, options) {
	...
	return vogels.access.query('Entitizer_EntityName', options)
		.then(function(result) {
			if (result && result.length > 0) {
 ...
 * @param {String} key - Entity's name key
...
AccessService.prototype.entityByName = function(data, options) {
	var key = EntityName.createKey(data.name, data.lang, data.country);
	return this.entityByKey(key, options);
};
