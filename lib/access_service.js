 ...
 * @param {Object} params.params - DynamoDB params
 */
AccessService.prototype.entityNameKeyByKey = function(key, options) {
	options = _.defaults(options, {
		format: 'json'
	});
	return vogels.access.getItem('EntityNameKey', key, options);
};
 ...
 * @param {Object} params.params - DynamoDB params
 */
AccessService.prototype.entitiesNameKeysByKeys = function(keys, options) {
	options = _.defaults(options, {
		format: 'json'
	});
	return vogels.access.getItems('EntityNameKey', keys, options);
};
...
AccessService.prototype.entitiesIdsByEnglishWikiId = function(id, options) {
	options = _.defaults(options, {
		format: 'items',
		limit: 100
	});
 ...
 */
AccessService.prototype.entityByNameKey = function(key, options) {
	var self = this;
	return self.entityNameKeyByKey(key)
 ...
 */
AccessService.prototype.entitiesByNameKeys = function(keys, options) {
	var self = this;

	return this.entitiesNameKeysByKeys(keys)
		.then(function(result) {
			var data = {};
