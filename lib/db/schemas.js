...
var GlobalEntitySchema = _.assign({}, RootEntitySchema, {
	...
	entities: Joi.object().pattern(/^[a-z]{2}$/, Joi.number().integer()).min(1).required(),
	// names by language code: {ro: {name: 'Name', abbr: 'NU'}, en: {name: 'Name'}}
	names: Joi.object({
		en: CultureNameSchema
	}).pattern(/^[a-z]{2}$/, CultureNameSchema).min(1).required()
});

var UpdateGlobalEntitySchema = {
	id: Joi.number().integer().required(),
	// md5 (COUNTRY_LANGUAGE_SLUG):
	slug_key: Joi.string().trim().length(32).lowercase(),
	...
	description: Joi.string().trim().max(400),
	// entities ids by country code: {md: 23243, us: 4235}
	entities: Joi.object().pattern(/^[a-z]{2}$/, Joi.number().integer()).min(1),
	// names by language code: {ro: {name: 'Name', abbr: 'NU'}, en: {name: 'Name'}}
	names: Joi.object({
		en: CultureNameSchema
	}).pattern(/^[a-z]{2}$/, CultureNameSchema).min(1)
};
...
var BasicEntitySchema = _.assign({}, RootEntitySchema, {
	...
	names: Joi.array().items(Joi.object().keys(EntityNameSchema)).min(1).max(50).required()
});

var EntitySchema = _.assign({}, BasicEntitySchema, {});

// function createUpdateSchema(schema, id) {
// 	id = id || 'id';
// 	var updated = {};
// 	var value;
// 	for (var prop in schema) {
// 		value = _.clone(schema[prop]);
// 		value = updated[prop] = _.assign(Joi.string(), value);
// 		if (value._flags.presence === 'required') {
// 			value.invalid(null);
// 			if (id !== prop) {
// 				delete value._flags.presence;
// 			}
// 		}
...
// var UpdateEntitySchema = createUpdateSchema(EntitySchema);
// normalizeUpdate(UpdateEntitySchema);
// console.log(UpdateEntitySchema);
...
exports.UpdateGlobalEntitySchema = UpdateGlobalEntitySchema;
